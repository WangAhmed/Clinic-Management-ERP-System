...
namespace Appointment.Controllers
{
    public class s_PrescriptionController : Controller
    {
        ...
        {
            // Retrieve the UserID from the session and cast it to an integer
            ...
            // Assuming that the UserID is the DoctorID in this context
            int doctorId = userId.Value;

            // Filter prescriptions based on the logged-in DoctorID
            var s_Prescription = db.s_Prescription
                .Include(s => s.m_medicines)
                .Include(s => s.s_Appointment)
                .Where(p => p.s_Appointment.DoctorID == doctorId)
                .ToList();
